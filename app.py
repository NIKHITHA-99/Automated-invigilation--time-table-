...
class Invigilator:
    ...
    def is_available(self, exam_day: str, exam_start: str, exam_end: str) -> bool:
        ...
        for invigilation in self.invigilations:
            if invigilation['day'].lower() == exam_day and self._time_overlap(
                (exam_start, exam_end),
                (invigilation['start_time'], invigilation['end_time'])  # Already time objects
            ):
                return False
    ...
    @staticmethod
    def _str_to_time(time_str) -> time:
    ...
    def assign_exam(self, exam_data: Dict) -> None:
        ...
        # ✅ FIX: Store time objects instead of raw strings to allow comparison later
        self.invigilations.append({
            **exam_data,
            'start_time': start,
            'end_time': end
        })
    ...
    try:
        ...
        for _, row in invig_df.iterrows():
            ...
            invigilators.append(Invigilator(
                ...
                max_hours=float(pd.to_numeric(row['max_hours'], errors='coerce') or 0),
        ...
        for _, row in exams_df.iterrows():
            exams.append(Exam(
                ...
                duration=float(pd.to_numeric(row['duration'], errors='coerce') or 0)
            ))
...
def sample_files():
    # Sample exam data
    exam_data = """day,date,subject,form,start_time,duration,required
